import { _decorator, Component, Node , Prefab , CCFloat , CCInteger , ScrollView , Layout} from 'cc';
...
export class ScrollList extends Component {
    ...
    private templateType = TemplateType.NODE

    @property({type : Node , visible : function (){
    ...
    private tmpPrefab = null

    @property({type : SlideType})
    private slideMode : SlideType = 1

    @property({type : CCFloat , visible: function () {
            return this.slideMode == SlideType.PAGE;
        } , slide: true , range: [0, 1, .1]})
    private pageDistance = 3

    @property({type : Component.EventHandler , visible: function () {
            let bool = this.slideMode == SlideType.PAGE;
            if (!bool)
                this.pageChangeEvent = null;
            return bool;
        },})
    private pageChangeEvent = null

    @property({tooltip : "是否为虚拟列表（动态列表）"})
    private virtual = false

    @property({tooltip : "是否为循环列表" , visible: function () {
    ...
    @property({tooltip : "Item数量不足以填满Content时，是否居中显示Item（不支持Grid布局）" , visible: function () {
            return this.virtual;
        }})
    private lackCenter = false

    @property({tooltip: 'Item数量不足以填满Content时，是否可滑动',
        visible: function () {
    ...
    private lackSlide = false

    @property({type: CCInteger,
    range: [0, 6, 1],
    tooltip: '刷新频率（值越大刷新频率越低、性能越高）',
    slide: true})
    private updateRate : CCInteger = 0

    @property({type: CCInteger,
        range: [0, 12, 1],
        tooltip: '逐帧渲染时，每帧渲染的Item数量（<=0时关闭分帧渲染）',
        slide: true})
    private frameByFrameRenderNum : CCInteger = 0;

    @property({type : Component.EventHandler , tooltip: '渲染事件（渲染器）' , visible : false})
    private renderEvent
    ...
    @property({type : SelectedType ,  tooltip: '选择模式'})
    private selectedMode = SelectedType.NONE

    @property({tooltip: '是否重复响应单选事件',
        visible: function () {
            return this.selectedMode == SelectedType.SINGLE;
        }})
    private repeatEventSingle = false

    @property({type: Component.EventHandler,
        tooltip: '触发选择事件',
        visible: function () {
    ...
    private _lastSelectedId = -1;

    private _selectedId = -1
    public get selectedId(){
        return this._selectedId
    }
    public set selectedId(val){
        let t = this;
        let item
        switch (t.selectedMode){
            case SelectedType.SINGLE: {
                ...
                if (t.selectedEvent) {
                    Component.EventHandler.emitEvents([t.selectedEvent], item, val % this._actualNumItems, t._lastSelectedId % this._actualNumItems);
                }
                break;
            }
            case SelectedType.MULT: {
                ...
                if (t.selectedEvent) {
                    Component.EventHandler.emitEvents([t.selectedEvent], item, val % this._actualNumItems, t._lastSelectedId % this._actualNumItems, bool);
                }
                break;
            }
        }
    ...
    _init() {
        ...
        if(this.renderEventTarget != null){
            ...
            this.renderEvent.handler = "OnScrollList";
            // this.renderEvent.customEventData = "my data";
        }
    ...
    _createOrUpdateItem(data) {
        ...
        if (!item) { //如果不存在
            ...
            if (canGet) {
                item = this._pool.get();
                //('从池中取出::   旧id =', item._listId, '，新id =', data.id, item);
            } else {
                item = cc.instantiate(this._itemTmp);
                //('新建::', data.id, item);
            }
            item._listId = data.id;
            item.setPosition(new cc.v3(data.x, data.y , 0));
    ...
    _delRedundantItem() {
        if (this.virtual) {
            ...
            // ('存入::', str, '    pool.length =', this._pool.length);
        } else {
    ...
    /**
     * 动效删除Item（此方法只适用于虚拟列表，即virtual=true）
     * 一定要在回调函数里重新设置新的numItems进行刷新，毕竟本List是靠数据驱动的。
     */
    aniDelItem(listId, callFunc, aniType) {
        let t = this;

        if (!t.checkInited() || t.cyclic || !t.virtual)
    ...
    scrollTo(listId, timeInSecond, offset, overStress) {
        ...
        if (!t.virtual && t._layout && t._layout.enabled)
        ...
        // (runScroll, t._scrollPos, viewPos, comparePos)
    ...
    _calcNearestItem() {
        ...
        for (let n = 0; n < t.content.childrenCount && !breakFor; n += t._colLineNum) {
            data = this.virtual ? this.displayData[n] : this._calcExistItemPos(n);
        ...
        data = this.virtual ? this.displayData[this.displayItemNum - 1] : this._calcExistItemPos(this._numItems - 1);
